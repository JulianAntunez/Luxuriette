...
async function pay() {
    try {
        const response = await fetch("/api/pay", {
            method: 'POST',
            body: JSON.stringify(carrito.map(item => item.id)), // Solo enviar IDs
            headers: {
...
// Llama a la función de auto actualización al cargar la página
window.onload = async () => {
    await fetchProducts();
    loadCart(); // Cargar el carrito al iniciar
    autoUpdateCart(); // Iniciar la actualización automática del carrito
};